...
func _process(_delta):
	if Input.is_action_pressed("fire") and can_fire and is_network_master():
		rpc("shoot", $BulletPoint.global_position, rotation_degrees, global_rotation)
		can_fire = false
		yield(get_tree().create_timer(fire_rate), "timeout")
		can_fire = true


remotesync func shoot(bullet_position, bullet_rotation_degrees, player_global_rotation):
	$Gunshot.play()
